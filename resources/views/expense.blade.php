...
<script>
    ...
    jQuery("#search_date").click(function() {
        ...
        if (jQuery("#datefrom").val())
            f.rules.push({field:"expenses.created_at",op:"ge",data:jQuery("#datefrom").val()});

        if (jQuery("#dateto").val())
            f.rules.push({field:"expenses.created_at",op:"le",data:jQuery("#dateto").val()});

        var s = {groupOp:"OR",rules:[],groups:[f]};
        s.rules.push({field:"expenses.created_at",op:"nu",data:''});
